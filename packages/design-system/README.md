...
> 一套可跨项目复用的移动端 Web/PWA 设计系统（devkit · `packages/design-system`）。
> 目标气质：**Apple Notes × Apple Podcasts × Safari 阅读模式**——安静、克制、内容优先，像原生 App，而不是传统 SaaS。
>
> 本包附带可直接拷贝的代码骨架：`tokens.css` + `tailwind.config.ts` + `globals.css`。
> 用法见 §14；这是 devkit 套件的一个 package，根级说明见仓库 `README.md`。
